...
async function fetchGitHubProjects(username = 'DimePrangija') {
  try {
    ...
    return null; // Return null on error to use fallback
  }
}

// Function to render GitHub projects (can be called to update project section)
function renderGitHubProjects(projects, containerSelector = '#projects .grid, #projects .projects-grid') {
  if (!projects || projects.length === 0) {
    console.log('No GitHub projects to display, using static content');
    return;
  }
  ...
  // Create project cards from GitHub data
  // Note: This function can be extended to replace or supplement existing project cards
  projects.forEach(project => {
    console.log('GitHub Project:', project);
  });
}

// Fetch and log GitHub projects on page load (for now, just log them)
// In a full implementation, this would replace or supplement the static project cards
document.addEventListener('DOMContentLoaded', async () => {
  // Optionally fetch GitHub projects
  // Uncomment to enable:
  // const githubProjects = await fetchGitHubProjects('DimePrangija');
  // if (githubProjects) {
  //   console.log('Fetched GitHub projects:', githubProjects);
  //   // renderGitHubProjects(githubProjects);
  // }
});